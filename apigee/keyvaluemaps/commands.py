...
from apigee.encryption_utils import (ENCRYPTED_HEADER_BEGIN, ENCRYPTED_HEADER_END, decrypt_message_with_gpg, encrypt_message_with_gpg, has_encrypted_header)
...
def _create_keyvaluemap_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, body, **kwargs):
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).create_keyvaluemap_in_an_environment(environment,
                                                                                                                                             body).text
    )
...
def _delete_keyvaluemap_from_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_keyvaluemap_from_an_environment(environment).text
    )
...
def _delete_keyvaluemap_entry_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, entry_name, **kwargs):
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org,
                   name).delete_keyvaluemap_entry_in_an_environment(environment, entry_name).text
    )
...
def _get_keyvaluemap_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_keyvaluemap_in_an_environment(environment).text
    )
...
def _get_a_keys_value_in_an_environment_scoped_keyvaluemap(
  username, password, mfa_secret, token, zonename, org, profile, name, environment, entry_name, **kwargs
    ...
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org,
                   name).get_a_keys_value_in_an_environment_scoped_keyvaluemap(environment, entry_name).text
    )
...
def _list_keyvaluemaps_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, environment, prefix=None, **kwargs):
    return Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org,
...
def _update_keyvaluemap_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, body, **kwargs):
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).update_keyvaluemap_in_an_environment(environment,
                                                                                                                                             body).text
    )
...
def _create_an_entry_in_an_environment_scoped_kvm(
  username, password, mfa_secret, token, zonename, org, profile, name, environment, entry_name, entry_value, **kwargs
    ...
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org,
                   name).create_an_entry_in_an_environment_scoped_kvm(environment, entry_name, entry_value).text
    )
...
def _update_an_entry_in_an_environment_scoped_kvm(
  username, password, mfa_secret, token, zonename, org, profile, name, environment, entry_name, updated_value, **kwargs
    ...
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org,
                   name).update_an_entry_in_an_environment_scoped_kvm(environment, entry_name, updated_value).text
    )
...
def _list_keys_in_an_environment_scoped_keyvaluemap(
  username, password, mfa_secret, token, zonename, org, profile, name, environment, startkey, count, **kwargs
    ...
    return (
      Keyvaluemaps(generate_authentication(username, password, mfa_secret, token, zonename), org,
                   name).list_keys_in_an_environment_scoped_keyvaluemap(environment, startkey, count).text
    )
...
def encrypt_file(symmetric_key, file, verbose, silent):
    ...
    contents, encrypted_count = Keyvaluemaps.encrypt_keyvaluemap(contents, symmetric_key)
    if encrypted_count:
