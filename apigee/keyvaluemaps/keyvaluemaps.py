...
class _RequestHelper:
    ...
    def post(auth_obj, uri, json_body=None):
        resp = requests.post(
            ...
            headers=auth.set_authentication_headers(
                auth_obj, custom_headers={"Accept": "application/json", "Content-Type": "application/json"}
            ),
            json=json_body,
        )
    ...
    def get(auth_obj, uri):
        resp = requests.get(
            uri,
            headers=auth.set_authentication_headers(auth_obj, custom_headers={"Accept": "application/json"}),
        )
        resp.raise_for_status()
        return resp
...
class _SyncHelper:
    ...
    def find_deleted_keys(local_map, remote_map):
        return [
            entry
            for entry in remote_map.get("entry", [])
            if entry["name"] not in {e["name"] for e in local_map.get("entry", [])}
        ]

    @staticmethod
    def entries_to_update(local_map, remote_map):
        return {"entry": [entry for entry in local_map.get("entry", []) if entry not in remote_map.get("entry", [])]}
...
class Keyvaluemaps:
    ...
    def create_an_entry_in_an_environment_scoped_kvm(self, environment, entry_name, entry_value):
        uri = CREATE_AN_ENTRY_IN_AN_ENVIRONMENT_SCOPED_KVM_PATH.format(
            api_url=APIGEE_ADMIN_API_URL, org=self.org_name, environment=environment, name=self.map_name
        )
        return _RequestHelper.post(self.auth, uri, json_body={"name": entry_name, "value": entry_value})
    ...
    def update_an_entry_in_an_environment_scoped_kvm(self, environment, entry_name, updated_value):
        ...
        return _RequestHelper.post(self.auth, uri, json_body={"name": entry_name, "value": updated_value})
    ...
    def delete_keyvaluemap_entry_in_an_environment(self, environment, entry_name):
        uri = DELETE_KEYVALUEMAP_ENTRY_IN_AN_ENVIRONMENT_PATH.format(
            api_url=APIGEE_ADMIN_API_URL, org=self.org_name, environment=environment, name=self.map_name, entry_name=entry_name
        )
        return self.send_delete_request(uri)

    # ---- KVM CRUD ----
    def create_keyvaluemap_in_an_environment(self, environment, request_body):
        uri = CREATE_KEYVALUEMAP_IN_AN_ENVIRONMENT_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, environment=environment)
        return _RequestHelper.post(self.auth, uri, json_body=json.loads(request_body))
    ...
    def list_keys_in_an_environment_scoped_keyvaluemap(self, environment, startkey, count):
        uri = LIST_KEYS_IN_AN_ENVIRONMENT_SCOPED_KEYVALUEMAP_PATH.format(
            api_url=APIGEE_ADMIN_API_URL, org=self.org_name, environment=environment, name=self.map_name, startkey=startkey, count=count
        )
        return self.fetch_keys_in_environment_scoped_keyvaluemap(uri)

    def list_keyvaluemaps_in_an_environment(self, environment, prefix=None, format="json"):
        uri = LIST_KEYVALUEMAPS_IN_AN_ENVIRONMENT_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, environment=environment)
    ...
    def synchronize_keyvaluemap_with_environment(self, environment, local_map):
        ...
        entries_to_update = _SyncHelper.entries_to_update(local_map, remote_map)

        if deleted_keys:
            self.delete_entries(environment, deleted_keys)
            console.echo("Removed entries.")
        if entries_to_update["entry"]:
            for entry in tqdm(entries_to_update["entry"], **get_tqdm_kwargs("Updating")):
    ...
    def push_keyvaluemap(self, environment, file, secret=None):
        ...
        if secret:
            ...
            local_map, decrypted_count = Keyvaluemaps.decrypt_keyvaluemap(local_map, secret)
            console.echo("Done." if decrypted_count else "Nothing to decrypt.")
        elif any(has_encrypted_header(entry.get("value")) for entry in local_map.get("entry", [])):
            sys.exit("KVM appears to be encrypted but no symmetric key (secret) was specified.")
        ...
        try:
            ...
            console.echo(self.create_keyvaluemap_in_an_environment(environment, json.dumps(local_map)).text)
