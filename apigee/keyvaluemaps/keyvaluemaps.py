...
class Keyvaluemaps:
    ...
    def create_an_entry_in_an_environment_scoped_kvm(self, environment, entry_name, entry_value):  # sourcery skip: class-extract-method
    ...
    def fetch_keys_in_environment_scoped_keyvaluemap(self, uri):
        result = requests.get(uri, headers=auth.set_authentication_headers(self.auth, custom_headers={"Accept": "application/json"}))
        result.raise_for_status()
        return result

    @staticmethod
    def find_deleted_keys(kvm_dict1, kvm_dict2):
        return [entry for entry in kvm_dict2["entry"] if entry["name"] not in {entry["name"] for entry in kvm_dict1["entry"]}]
    ...
    def send_delete_request(self, uri):
        resp = requests.delete(uri, headers=auth.set_authentication_headers(self.auth, custom_headers={"Accept": "application/json"}))
        resp.raise_for_status()
        return resp
    ...
    def synchronize_keyvaluemap_with_environment(self, environment, local_map):
        ...
        entries_to_update = {"entry": [entry for entry in local_map["entry"] if entry not in remote_map["entry"]]}
        if deleted_keys:
