...
class Maskconfigs:
    ...
    def create_data_masks_for_an_api_proxy(self, request_body):
        uri = CREATE_DATA_MASKS_FOR_AN_API_PROXY_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, api_name=self.api_name)
    ...
    def list_data_masks_for_an_api_proxy(self):
        uri = LIST_DATA_MASKS_FOR_AN_API_PROXY_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, api_name=self.api_name)
        return self.fetch_data_masks_for_organization(uri)

    def list_data_masks_for_an_organization(self):
        uri = LIST_DATA_MASKS_FOR_AN_ORGANIZATION_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name)
        return self.fetch_data_masks_for_organization(uri)
