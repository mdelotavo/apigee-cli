...
from apigee import (
  APIGEE_CLI_ACCESS_TOKEN_FILE, APIGEE_CLI_CREDENTIALS_FILE, APIGEE_CLI_DIRECTORY, APIGEE_CLI_IS_MACHINE_USER, APIGEE_CLI_REFRESH_TOKEN_FILE, APIGEE_OAUTH_URL,
...
def generate_authentication_error_message(authentication_error):
    ...
    return (f"{error_message} \nWARNING: APIGEE_CLI_IS_MACHINE_USER={APIGEE_CLI_IS_MACHINE_USER}" if APIGEE_CLI_IS_MACHINE_USER else error_message)
...
def get_access_token_for_sso(auth, username, password, oauth_url, post_headers, session, passcode):
...
def set_authentication_headers(authentication_object, custom_headers=None):
    ...
    if authentication_object.mfa_secret or authentication_object.token or authentication_object.zonename:
        ...
        custom_headers["Authorization"] = (
          "Basic " + base64.b64encode((f"{authentication_object.username}:{authentication_object.password}").encode()).decode()
        )
...
def get_access_token_command(username, password, mfa_secret, token, zonename, org, profile, passcode, **kwargs):
...
@common_verbose_options
@common_silent_options
def view_access_token(username, password, mfa_secret, token, zonename, org, profile, **kwargs):
