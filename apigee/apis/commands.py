...
def _delete_api_proxy_revision(
  ...
  **kwargs,
    ...
    return (Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).delete_api_proxy_revision(name, revision_number).text)
...
def _deploy_api_proxy_revision(
  ...
  **kwargs,
    ...
    return (
      Apis(generate_authentication(username, password, mfa_secret, token, zonename),
           org).deploy_api_proxy_revision(name, environment, revision_number, delay=delay, override=override).text
    )


@apis.command(help="Deploys a revision of an existing API proxy to an environment in an organization.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
...
def _delete_undeployed_revisions(
  ...
  **kwargs,
    ...
    return Apis(generate_authentication(username, password, mfa_secret, token, zonename),
...
def _delete_api_proxy(
  ...
  **kwargs,
    ...
    return Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).delete_api_proxy(name)


@apis.command(help="Deletes an API proxy and all policies, resources, endpoints, and revisions associated with it.")
@common_auth_options
@common_verbose_options
@common_silent_options
@click.option("-n", "--name", help="name", required=True)
...
def _export_api_proxy(
  ...
  **kwargs,
    ...
    return Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).export_api_proxy(
      name,
      revision_number,
      write_to_filesystem=True,
...
def _get_api_proxy(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).get_api_proxy(name).text)
...
def _list_api_proxies(
  ...
  **kwargs,
    ...
    return Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).list_api_proxies(prefix=prefix)
...
def _list_api_proxy_revisions(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).list_api_proxy_revisions(name).text)
...
def _undeploy_api_proxy_revision(
  ...
  **kwargs,
    ...
    return (
      Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).undeploy_api_proxy_revision(name, environment, revision_number).text
    )
...
def _force_undeploy_api_proxy_revision(
  ...
  **kwargs,
    ...
    return (
      Apis(generate_authentication(username, password, mfa_secret, token, zonename), org).force_undeploy_api_proxy_revision(name, environment,
