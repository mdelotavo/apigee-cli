...
from apigee.utils import (
  apply_function_on_iterable, check_file_exists, check_files_exist, create_directory, execute_function_on_directory_files, extract_zip_file
)
...
class ApiPullerWithDependencyExtraction:
    ...
    def export_and_extract_api_proxy(self, api_name, force=False):
        ...
        exported_api = Apis(self.auth, self.org_name).export_api_proxy(api_name, self.revision_number, write_to_filesystem=True, output_file=self.zip_file)
    ...
    def export_cache_dependencies(self, environment, caches, force=False, expected_verbosity=1):
        ...
        def _func(cache):
            ...
            resp = (Caches(self.auth, self.org_name, cache).get_information_about_a_cache(environment).text)
    ...
    def export_keyvaluemap_dependencies(self, environment, keyvaluemaps, force=False, expected_verbosity=1):
        ...
        def _func(kvm):
            ...
            resp = (Keyvaluemaps(self.auth, self.org_name, kvm).get_keyvaluemap_in_an_environment(environment).text)
            console.echo(resp, expected_verbosity=1)
    ...
    def export_resource_dependencies_using_getattr(self, resource_type, files, environment, force=True):
        ...
        getattr(self, f"export_{resource_type}_dependencies")(environment, resource_files, force=force)
        return resource_files

    def export_targetserver_dependencies(self, environment, targetservers, force=False, expected_verbosity=1):
        ...
        def _func(ts):
            ...
            resp = (Targetservers(self.auth, self.org_name, ts).get_targetserver(environment).text)
            console.echo(resp, expected_verbosity=1)
