...
class Apis:
    ...
    def delete_undeployed_revisions(self, name, save_last=0, dry_run=False):
        ...
        deployed = ApisSerializer.filter_deployed_revisions(ApisSerializer.filter_deployment_details(details))
    ...
    def deploy_api_proxy_revision(self, name, env, revision, delay=0, override=False):
        return self._request(
          "post",
          DEPLOY_API_PROXY_REVISION_PATH.format(org=self.org, environment=env, api_name=name, revision_number=revision) + f"?delay={delay}",
    ...
    def force_undeploy_api_proxy_revision(self, name, env, revision):
        return self._request(
          "post",
          FORCE_UNDEPLOY_API_PROXY_REVISION_PATH.format(org=self.org, api_name=name, revision_number=revision),
          params={
            "action": "undeploy",
            "env": env,
            "force": "true"
    ...
    def undeploy_api_proxy_revision(self, name, env, revision):
        return self._request(
          "delete",
          UNDEPLOY_API_PROXY_REVISION_PATH.format(org=self.org, environment=env, api_name=name, revision_number=revision),
        )
