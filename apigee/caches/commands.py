...
def _clear_all_cache_entries(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, name).clear_all_cache_entries(environment).text)
...
def _clear_a_cache_entry(username, password, mfa_secret, token, zonename, org, profile, name, environment, entry, **kwargs):
    return (Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, name).clear_a_cache_entry(environment, entry).text)
...
def _create_a_cache_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, body, **kwargs):
    return (
      Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, name).create_a_cache_in_an_environment(environment, body).text
    )
...
def _get_information_about_a_cache(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_information_about_a_cache(environment).text)
...
def _list_caches_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, environment, prefix=None, **kwargs):
    return Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, None).list_caches_in_an_environment(environment, prefix=prefix)
...
def _update_a_cache_in_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, body, **kwargs):
    return (
      Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, name).update_a_cache_in_an_environment(environment, body).text
    )
...
def _delete_a_cache(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_a_cache(environment).text)
...
def _push_cache(username, password, mfa_secret, token, zonename, org, profile, environment, file, **kwargs):
    return Caches(generate_authentication(username, password, mfa_secret, token, zonename), org, None).push_cache(environment, file)
