...
class Userroles:
    ...
    def add_permissions_for_a_resource_to_a_user_role(self, request_body):
        return self.add_permissions_for_multiple_resources_to_user_role(ADD_PERMISSIONS_FOR_A_RESOURCE_TO_A_USER_ROLE_PATH, request_body)
    ...
    def add_permissions_for_multiple_resources_to_user_role(self, endpoint_template, request_body):
        ...
        return self.create_user_role_in_organization(uri, hdrs, body)

    def create_a_user_role_in_an_organization(self, roles):
        uri = CREATE_A_USER_ROLE_IN_AN_ORGANIZATION_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name)
        hdrs = auth.set_authentication_headers(
          self.auth,
          custom_headers={
            "Accept": "application/json",
    ...
    def delete_a_permission_for_a_resource(self, permission, resource_path):
        ...
        return self.remove_user_membership_from_role("application/octet-stream", uri)

    def delete_a_user_role(self):
        uri = DELETE_A_USER_ROLE_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, role_name=self.role_name)
        return self.remove_user_membership_from_role("application/x-www-form-urlencoded", uri)
    ...
    def get_a_role(self):
        uri = GET_A_ROLE_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, role_name=self.role_name)
        return self.verify_and_fetch_user_role_membership("application/json", uri)

    def get_resource_permissions_for_a_specific_role(self, resource_path=""):
        uri = GET_RESOURCE_PERMISSIONS_FOR_A_SPECIFIC_ROLE_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, role_name=self.role_name)
        if resource_path:
            uri += f"?path={resource_path}"
        return self.verify_and_fetch_user_role_membership("application/octet-stream", uri)

    def get_users_for_a_role(self):
        uri = GET_USERS_FOR_A_ROLE_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, role_name=self.role_name)
        return self.verify_and_fetch_user_role_membership("application/json", uri)

    def list_permissions_for_a_resource(self):
        uri = LIST_PERMISSIONS_FOR_A_RESOURCE_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, role_name=self.role_name)
        return self.verify_and_fetch_user_role_membership("application/json", uri)

    def list_user_roles(self):
        uri = LIST_USER_ROLES_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name)
        return self.verify_and_fetch_user_role_membership("application/json", uri)
    ...
    def remove_user_membership_in_role(self, user_email):
        ...
        return self.remove_user_membership_from_role("application/json", uri)
