...
class Backups:
    ...
    def list_api_proxies(self):
        return Apis(self.backupConfig.authentication, self.backupConfig.org_name).list_api_proxies(prefix=self.backupConfig.prefix, format="dict")

    def get_api_details(self, api_proxies):
        apis = {}
        for api in api_proxies:
            api_details = Apis(self.backupConfig.authentication, self.backupConfig.org_name).get_api_proxy(api).json()
    ...
    def list_api_products(self):
        return Apiproducts(self.backupConfig.authentication, self.backupConfig.org_name, None).list_api_products(prefix=self.backupConfig.prefix, format="dict")
    ...
    def list_all_developers(self, expand=False, count=1000, startkey=""):
        return Developers(self.backupConfig.authentication, self.backupConfig.org_name, None).list_developers(
          prefix=None,
          expand=expand,
          count=count,
          startkey=startkey,
    ...
    def list_apps_for_developers(self, developers):
        return Apps(self.backupConfig.authentication, self.backupConfig.org_name, None).list_apps_for_all_developers(
          developers,
    ...
    def list_caches_in_environment(self, environment):
        return Caches(self.backupConfig.authentication, self.backupConfig.org_name,
                      None).list_caches_in_an_environment(environment, prefix=self.backupConfig.prefix, format="dict")
    ...
    def create_developers_snapshot(self):
        self.backupConfig.snapshot_data.developers = Developers(self.backupConfig.authentication, self.backupConfig.org_name,
                                                                None).list_developers(prefix=self.backupConfig.prefix, format="dict")
    ...
    def list_keyvaluemaps_in_environment(self, environment):
        return Keyvaluemaps(self.backupConfig.authentication, self.backupConfig.org_name,
                            None).list_keyvaluemaps_in_an_environment(environment, prefix=self.backupConfig.prefix, format="dict")
    ...
    def list_targetservers_in_environment(self, environment):
        return Targetservers(self.backupConfig.authentication, self.backupConfig.org_name,
                             None).list_targetservers_in_an_environment(environment, prefix=self.backupConfig.prefix, format="dict")
    ...
    def list_user_roles(self):
        return (Userroles(self.backupConfig.authentication, self.backupConfig.org_name, None).list_user_roles().json())

    def filter_userroles(self, userroles):
        return ([role for role in userroles if role.startswith(self.backupConfig.prefix)] if self.backupConfig.prefix else userroles)
    ...
    def get_output_file_path(self, api, revision):
        return str(Path(self.backupConfig.working_org_directory) / "apis" / api / revision / f"{api}.zip")
    ...
    def get_apiproduct_content(self, apiproduct):
        content = (Apiproducts(self.backupConfig.authentication, self.backupConfig.org_name, apiproduct).get_api_product().text)
        return content
    ...
    def get_developer_app_details(self, developer, app):
        content = (Apps(self.backupConfig.authentication, self.backupConfig.org_name, app).get_developer_app_details(developer).text)
        return content
    ...
    def get_cache_information(self, environment, cache):
        content = (Caches(self.backupConfig.authentication, self.backupConfig.org_name, cache).get_information_about_a_cache(environment).text)
        return content
    ...
    def get_developer_details(self, developer):
        content = (Developers(self.backupConfig.authentication, self.backupConfig.org_name, developer).get_developer().text)
        return content
    ...
    def get_keyvaluemap_content(self, environment, kvm):
        content = (Keyvaluemaps(self.backupConfig.authentication, self.backupConfig.org_name, kvm).get_keyvaluemap_in_an_environment(environment).text)
        return content
    ...
    def get_targetserver_content(self, environment, targetserver):
        content = (Targetservers(self.backupConfig.authentication, self.backupConfig.org_name, targetserver).get_targetserver(environment).text)
        return content

    def get_targetserver_path(self, environment, targetserver):
        return TARGETSERVERS_TARGET_SUBPATH.format(environment=environment, targetserver=targetserver)
