...
def _create_developer_app(username, password, mfa_secret, token, zonename, org, profile, name, developer, body, **kwargs):
    return (Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, None).create_developer_app(developer, body).text)
...
def _delete_developer_app(username, password, mfa_secret, token, zonename, org, profile, name, developer, **kwargs):
    return (Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_developer_app(developer).text)
...
def _create_empty_developer_app(username, password, mfa_secret, token, zonename, org, profile, name, developer, display_name="", callback_url="", **kwargs):
    return (
      Apps(generate_authentication(username, password, mfa_secret, token, zonename), org,
           name).create_empty_developer_app(developer, display_name=display_name, callback_url=callback_url).text
    )
...
def _get_developer_app_details(username, password, mfa_secret, token, zonename, org, profile, name, developer, **kwargs):
    return (Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_developer_app_details(developer).text)
...
def _list_org_apps(username, password, mfa_secret, token, zonename, org, profile, apptype, expand, rows, startkey, status, **kwargs):
    return (
      Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, None).list_org_apps(apptype, expand, rows, startkey, status).text
    )
...
def _get_org_app(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_org_app().text)
...
def _list_developer_apps(
  username, password, mfa_secret, token, zonename, org, profile, developer, prefix=None, expand=False, count=1000, startkey="", **kwargs
    ...
    return Apps(generate_authentication(username, password, mfa_secret, token, zonename), org,
                None).list_developer_apps(developer, prefix=prefix, expand=expand, count=count, startkey=startkey)
...
def _delete_key_for_a_developer_app(username, password, mfa_secret, token, zonename, org, profile, name, developer, consumer_key=None, **kwargs):
    return (
      Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_key_for_a_developer_app(
        developer,
...
def _update_key_for_a_developer_app(username, password, mfa_secret, token, zonename, org, profile, name, developer, consumer_key=None, action="", **kwargs):
    return (
      Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).update_key_for_a_developer_app(
        developer,
        consumer_key=consumer_key,
        action=action,
      ).text
...
def _create_a_consumer_key_and_secret(
  ...
  **kwargs
    ...
    return (
      Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, name).create_a_consumer_key_and_secret(
        developer,
        consumer_key=consumer_key,
...
def _restore_app(username, password, mfa_secret, token, zonename, org, profile, file, **kwargs):
    return (Apps(generate_authentication(username, password, mfa_secret, token, zonename), org, None).restore_and_update_app_details(file).text)
