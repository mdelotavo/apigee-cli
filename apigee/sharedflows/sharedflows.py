...
class Sharedflows:
    ...
    def delete_undeployed_revisions(self, shared_flow_name, save_last=0, dry_run=False):
        details = SharedflowsSerializer.filter_deployment_details(
          Deployments(self.auth, self.org_name, shared_flow_name).get_shared_flow_deployment_details().json()
        )
    ...
    def get_a_list_of_shared_flows(self, prefix=None):
        uri = GET_A_LIST_OF_SHARED_FLOWS_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name)
    ...
    def import_a_shared_flow(self, shared_flow_file, shared_flow_name):
        uri = IMPORT_A_SHARED_FLOW_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name)
    ...
    def get_a_shared_flow(self, shared_flow_name):
        return self.retrieve_shared_flow_revisions(GET_A_SHARED_FLOW_PATH, shared_flow_name)
    ...
    def get_shared_flow_deployments(self, shared_flow_name):
        return self.retrieve_shared_flow_revisions(GET_SHARED_FLOW_DEPLOYMENTS_PATH, shared_flow_name)

    def get_shared_flow_revisions(self, shared_flow_name):
        return self.retrieve_shared_flow_revisions(GET_SHARED_FLOW_REVISIONS_PATH, shared_flow_name)
    ...
    def export_shared_flow(self, shared_flow_name, revision_number, output_file, write_to_filesystem=True):
