...
def _create_permissions(username, password, mfa_secret, token, zonename, org, profile, name, body, **kwargs):
    return (Permissions(generate_authentication(username, password, mfa_secret, token, zonename), org, name).create_role_permissions(body).text)
...
def _team_permissions(username, password, mfa_secret, token, zonename, org, profile, name, file, placeholder_key=None, placeholder_value="", **kwargs):
    return (
      Permissions(generate_authentication(username, password, mfa_secret, token, zonename), org,
                  name).apply_team_permissions_template(file, placeholder_key=placeholder_key, placeholder_value=placeholder_value).text
    )
...
def _get_permissions(username, password, mfa_secret, token, zonename, org, profile, name, format, showindex=False, tablefmt="plain", **kwargs):
    return Permissions(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_permissions(
      formatted=True,
