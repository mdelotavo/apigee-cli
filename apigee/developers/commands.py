...
def _create_developer(
  username, password, mfa_secret, token, zonename, org, profile, name, first_name, last_name, user_name, attributes='{"attributes" : [ ]}', **kwargs
    ...
    return (
      Developers(generate_authentication(username, password, mfa_secret, token, zonename), org,
                 name).create_developer(first_name, last_name, user_name, attributes=attributes).text
    )
...
def _delete_developer(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_developer().text)


@developers.command(
  help=
  "Deletes a developer from an organization. All apps and API keys associated with the developer are also removed from the organization. All times in the response are UNIX times. To avoid permanently deleting developers and their artifacts, consider deactivating developers instead."
)
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _get_developer(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_developer().text)


@developers.command(
  help=
  "Returns the profile for a developer by email address or ID. All time values are UNIX time values. The profile includes the developer's email address, ID, name, and other information."
)
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _get_developer_by_app(username, password, mfa_secret, token, zonename, org, profile, app_name, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, None).get_developer_by_app(app_name).text)


@developers.command(
  help=
  "Gets the developer profile by app name. The profile retrieved is for the developer associated with the app in the organization. All time values are UNIX time values."
)
@common_auth_options
@common_verbose_options
@common_silent_options
...
def _list_developers(username, password, mfa_secret, token, zonename, org, profile, prefix=None, expand=False, count=1000, startkey="", **kwargs):
    return Developers(generate_authentication(username, password, mfa_secret, token, zonename), org,
                      None).list_developers(prefix=prefix, expand=expand, count=count, startkey=startkey)
...
def _set_developer_status(username, password, mfa_secret, token, zonename, org, profile, name, action, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).set_developer_status(action).text)
...
def _update_developer(username, password, mfa_secret, token, zonename, org, profile, name, body, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).update_developer(body).text)
...
def _update_a_developer_attribute(username, password, mfa_secret, token, zonename, org, profile, name, attribute_name, updated_value, **kwargs):
    return (
      Developers(generate_authentication(username, password, mfa_secret, token, zonename), org,
                 name).update_a_developer_attribute(attribute_name, updated_value).text
    )
...
def _delete_developer_attribute(username, password, mfa_secret, token, zonename, org, profile, name, attribute_name, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_developer_attribute(attribute_name).text)
...
def _get_all_developer_attributes(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_all_developer_attributes().text)
...
def _update_all_developer_attributes(username, password, mfa_secret, token, zonename, org, profile, name, body, **kwargs):
    return (Developers(generate_authentication(username, password, mfa_secret, token, zonename), org, name).update_all_developer_attributes(body).text)


@developers.command(
  help=
  "Updates or creates developer attributes. This API replaces the current list of attributes with the attributes specified in the request body. This lets you update existing attributes, add new attributes, or delete existing attributes by omitting them from the request body."
)
@common_auth_options
@common_verbose_options
@common_silent_options
