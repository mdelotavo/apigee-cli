...
def _list_all_keystores_and_truststores(username, password, mfa_secret, token, zonename, org, profile, environment, prefix=None, **kwargs):
    return Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org,
...
def _get_a_keystore_or_truststore(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_a_keystore_or_truststore(environment).text)
...
def _get_cert_details_from_a_keystore_or_truststore(username, password, mfa_secret, token, zonename, org, profile, name, environment, cert_name, **kwargs):
    return (
      Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org,
                name).get_cert_details_from_a_keystore_or_truststore(environment, cert_name).text
    )
...
def _get_all_certs_from_a_keystore_or_truststore(username, password, mfa_secret, token, zonename, org, profile, name, environment, prefix=None, **kwargs):
    return Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org,
                     name).get_all_certs_from_a_keystore_or_truststore(environment, prefix=prefix)
...
def _export_a_cert(username, password, mfa_secret, token, zonename, org, profile, name, environment, cert_name, **kwargs):
    return (Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org, name).export_a_cert(environment, cert_name).text)
...
def _list_aliases(username, password, mfa_secret, token, zonename, org, profile, name, environment, prefix=None, **kwargs):
    return Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org, name).list_aliases(environment, prefix=prefix)
...
def _get_alias(username, password, mfa_secret, token, zonename, org, profile, name, environment, alias_name, **kwargs):
    return (Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_alias(environment, alias_name).text)
...
def _export_a_certificate_for_an_alias(username, password, mfa_secret, token, zonename, org, profile, name, environment, alias_name, **kwargs):
    return (
      Keystores(generate_authentication(username, password, mfa_secret, token, zonename), org, name).export_a_certificate_for_an_alias(environment,
                                                                                                                                       alias_name).text
    )


@keystores.command(help="Exports a certificate or certificate chain for the specified alias in a keystore.")
@common_auth_options
@common_silent_options
@common_verbose_options
@click.option("-n", "--name", help="keystore name", required=True)
