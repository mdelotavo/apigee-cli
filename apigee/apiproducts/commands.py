...
def _create_api_product(username, password, mfa_secret, token, zonename, org, profile, name, body, **kwargs):
    return (Apiproducts(generate_authentication(username, password, mfa_secret, token, zonename), org, name).create_api_product(body).text)
...
def _delete_api_product(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Apiproducts(generate_authentication(username, password, mfa_secret, token, zonename), org, name).delete_api_product().text)
...
def _get_api_product(username, password, mfa_secret, token, zonename, org, profile, name, **kwargs):
    return (Apiproducts(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_api_product().text)
...
def _list_api_products(username, password, mfa_secret, token, zonename, org, profile, prefix=None, expand=False, count=1000, startkey="", **kwargs):
    return Apiproducts(generate_authentication(username, password, mfa_secret, token, zonename), org,
                       None).list_api_products(prefix=prefix, expand=expand, count=count, startkey=startkey)
...
def _update_api_product(username, password, mfa_secret, token, zonename, org, profile, name, body, **kwargs):
    return (Apiproducts(generate_authentication(username, password, mfa_secret, token, zonename), org, name).update_api_product(body).text)
...
def _push_apiproducts(username, password, mfa_secret, token, zonename, org, profile, file, **kwargs):
    return Apiproducts(generate_authentication(username, password, mfa_secret, token, zonename), org, None).push_apiproducts(file)
