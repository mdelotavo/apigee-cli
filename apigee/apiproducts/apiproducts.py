...
class Apiproducts:
    ...
    def create_api_product(self, request_body):
        uri = CREATE_API_PRODUCT_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name)
    ...
    def delete_api_product(self):
        uri = DELETE_API_PRODUCT_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, name=self.apiproduct_name)
    ...
    def get_api_product(self):  # sourcery skip: class-extract-method
        uri = GET_API_PRODUCT_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, name=self.apiproduct_name)
        hdrs = auth.set_authentication_headers(self.auth, custom_headers={"Accept": "application/json"})
    ...
    def update_api_product(self, request_body):
        uri = UPDATE_API_PRODUCT_PATH.format(api_url=APIGEE_ADMIN_API_URL, org=self.org_name, name=self.apiproduct_name)
        hdrs = auth.set_authentication_headers(
          self.auth,
          custom_headers={
            "Accept": "application/json",
