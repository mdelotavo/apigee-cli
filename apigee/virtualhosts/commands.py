...
@click.group(help="A named network configuration (including URL) for an environment (for example 'test' or 'prod') on API Services.")
def virtualhosts():
    pass
...
def _get_a_virtual_host_for_an_environment(username, password, mfa_secret, token, zonename, org, profile, name, environment, **kwargs):
    return (
      Virtualhosts(generate_authentication(username, password, mfa_secret, token, zonename), org, name).get_a_virtual_host_for_an_environment(environment).text
    )
...
def _list_virtual_hosts_for_an_environment(username, password, mfa_secret, token, zonename, org, profile, environment, prefix=None, **kwargs):
    return Virtualhosts(generate_authentication(username, password, mfa_secret, token, zonename), org,
